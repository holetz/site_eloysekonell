...
The platform — deploy targets, permissions/governance, external services, secrets
management.

**Boundary:** *where things run and who may touch them*; the *build/deploy path* that puts
code there lives in [../ci-cd/](../ci-cd/index.md). One standard per file (files, not
...
`deploy-targets` · `permissions-governance` · `external-services` · `secrets`.
